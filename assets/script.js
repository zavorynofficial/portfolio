(() => {
  ...
  function initGalleryPages(){
    ...
    const map={
      'logos.html':['assets/img/brand-designing/logos','Logo Design'],
      'letterheads.html':['assets/img/brand-designing/letterheads','Letterheads'],
      'business-cards.html':['assets/img/brand-designing/business-cards','Business Cards'],
      'stationery.html':['assets/img/brand-designing/stationery','Stationery'],
      'post-design.html':['assets/img/post-designing','Post Design'],
      'social-media.html':['assets/img/social-media-management','Social Media'],
      'ai-automation.html':['assets/img/ai-automation','AI & Automation']
    };
    const multi=[...document.querySelectorAll('[data-gallery-path]')];
    if(multi.length){
      multi.forEach(el=>renderGallery(el,el.dataset.galleryPath,el.dataset.galleryLabel||'Portfolio'));
      return;
    }
    const el=document.querySelector('[data-gallery]');
    if(el&&map[file])renderGallery(el,...map[file]);
  }
  ...
  document.addEventListener('DOMContentLoaded',()=>{injectStyle('zavoryn-quality-css','assets/brand-upgrades.css?v=20260925-audit4');if(!document.querySelector('.hero-premium'))injectStyle('zavoryn-global-css','assets/zavoryn-global.css?v=20260925-audit4');universalNav();universalFooter();a11y();makeLightbox();initNav();initHeader();initTracking();injectCardGraphics();initGoals();initFeatured();initGalleryPages();observe();const loader=document.createElement('script');loader.src='assets/production-upgrade.js';loader.async=true;document.head.appendChild(loader)});
